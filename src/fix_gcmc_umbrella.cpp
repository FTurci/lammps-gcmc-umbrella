...
FixGCMCUmbrella::FixGCMCUmbrella(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
...
void FixGCMCUmbrella::options(int narg, char **arg)
{
  ...
  if (strcmp(arg[iarg],"mol") == 0) {
    ...
    //  FT Umbrella
    else if (strcmp(arg[iarg],"umbrella") == 0) {
       if (iarg+2 > narg) error->all(FLERR,"Illegal fix gcmc/umbrella command");
      k = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      n0 = utils::numeric(FLERR,arg[iarg+2],false,lmp);
      iarg += 3;
      printf("The umbrella parameters are k = %g and n0 = %d.\n ",k,n0);
...
FixGCMCUmbrella::~FixGCMCUmbrella()
{
...
void FixGCMCUmbrella::init()
{
  ...
  if (full_flag && !exclusion_group_bit) {
    ...
    auto group_id = std::string("FixGCMCUmbrella:gcmc_exclusion_group:") + id;
  ...
  if (exchmode == EXCHMOL || movemode == MOVEMOL) {
    ...
    auto group_id = std::string("FixGCMCUmbrella:rotation_gas_atoms:") + id;
...
void FixGCMCUmbrella::pre_exchange()
{
...
void FixGCMCUmbrella::attempt_atomic_translation()
{
...
void FixGCMCUmbrella::attempt_atomic_deletion()
{
  ...
  if (i >= 0) {
    ...
    int n_before = atom->natoms;
    int n_after = atom->natoms-1;

    double umbr = -beta*0.5*k*((n_after-n0)*(n_after-n0)-(n_before-n0)*(n_before-n0));


    if (random_unequal->uniform() <
        ngas*exp(beta*deletion_energy)/(zz*volume)*exp(umbr) ){
...
void FixGCMCUmbrella::attempt_atomic_insertion()
{
  double lamda[3];
  ...
  if (proc_flag) {
    ...
    if (insertion_energy < MAXENERGYTEST &&
        random_unequal->uniform() <
        zz*volume*exp(-beta*insertion_energy)/(ngas+1)*exp(umbr) ){
...
void FixGCMCUmbrella::attempt_molecule_translation()
{
  ntranslation_attempts += 1.0;
...
void FixGCMCUmbrella::attempt_molecule_rotation()
{
...
void FixGCMCUmbrella::attempt_molecule_deletion()
{
  ndeletion_attempts += 1.0;
...
void FixGCMCUmbrella::attempt_molecule_insertion()
{
  double lamda[3];
  ninsertion_attempts += 1.0;
  ...
  //   // FT: to the total energy we add the umbrella contribution
  // double umbr = beta*0.5*k*(natoms_per_molecule-n0)*(natoms_per_molecule-n0);
  // insertion_energy += umbr;
...
void FixGCMCUmbrella::attempt_atomic_translation_full()
{
  ntranslation_attempts += 1.0;
...
void FixGCMCUmbrella::attempt_atomic_deletion_full()
{
  double q_tmp;
...
void FixGCMCUmbrella::attempt_atomic_insertion_full()
{
  double lamda[3];
  ninsertion_attempts += 1.0;
...
void FixGCMCUmbrella::attempt_molecule_translation_full()
{
  ntranslation_attempts += 1.0;
...
void FixGCMCUmbrella::attempt_molecule_rotation_full()
{
  nrotation_attempts += 1.0;
...
void FixGCMCUmbrella::attempt_molecule_deletion_full()
{
  ndeletion_attempts += 1.0;
...
void FixGCMCUmbrella::attempt_molecule_insertion_full()
{
  double lamda[3];
  ninsertion_attempts += 1.0;
...
double FixGCMCUmbrella::energy(int i, int itype, tagint imolecule, double *coord)
{
...
double FixGCMCUmbrella::molecule_energy(tagint gas_molecule_id)
{
...
double FixGCMCUmbrella::energy_full()
{
  int imolecule;
...
int FixGCMCUmbrella::pick_random_gas_atom()
{
  int i = -1;
...
tagint FixGCMCUmbrella::pick_random_gas_molecule()
{
  int iwhichglobal = static_cast<int> (ngas*random_equal->uniform());
...
void FixGCMCUmbrella::toggle_intramolecular(int i)
{
...
void FixGCMCUmbrella::update_gas_atoms_list()
{
...
double FixGCMCUmbrella::compute_vector(int n)
{
...
double FixGCMCUmbrella::memory_usage()
{
...
void FixGCMCUmbrella::write_restart(FILE *fp)
{
  int n = 0;
  double list[12];
...
void FixGCMCUmbrella::restart(char *buf)
{
  int n = 0;
...
void FixGCMCUmbrella::grow_molecule_arrays(int nmolatoms) {
